...
import io.vertx.workshop.map.source.SerializableMapSource;
...
public class MapRenderVerticle extends AbstractVerticle {
  ...
  static {
    try {
      ...
      final String index = System.getProperty("index", "amsterdam.osm.idx.gz");

      RuleSet ruleset = new RuleSet(MapRenderVerticle.class.getClassLoader().getResourceAsStream(rule));
      MapSource map = new SerializableMapSource(MapRenderVerticle.class.getClassLoader().getResourceAsStream(index));
